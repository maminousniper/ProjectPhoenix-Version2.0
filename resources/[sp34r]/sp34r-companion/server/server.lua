...
QBCore.Functions.CreateUseableItem(core_items.waterbottle.item_name, function(source, item)
    ...
    local water_bottle_refill_value = Config.core_items.waterbottle.settings.water_bottle_refill_value
    if Player == nil then return end
    if not remove_item(source, Player, 'water', water_bottle_refill_value) then
...
QBCore.Functions.CreateCallback('keep-companion:server:decrease_thirst', function(source, cb, data)
    ...
    if pet_water_bottle.metadata.liter == nil then
        TriggerClientEvent('QBCore:Notify', source, 'You should wash water bottle first!', 'error', 2500)
        print('maybe use your water bottle when there is some water s in your inventory')
...
RegisterNetEvent('keep-companion:server:revivePet', function(item, process_type)
    ...
    if petData.metadata.health >= pet_maxHealth then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Your dog is at full health again!", 'primary', 2500)
        return
    end
...
function getGloveItem(plate, item)
    local hasItem = exports.ox_inventory:Search('glove'..plate, 'count', item)
    if hasItem >= 1 then 
        return true
    end
    return 
...
function getTrunkItem(plate, item)
    local hasItemTrunk = exports.ox_inventory:Search('trunk'..plate, 'count', item)
    if hasItemTrunk >= 1 then 
        return true
    end
    return false
end
...
local function search_vehicle(Type, plate)
    local illegal = false
    if Type == 1 then
        if getGloveItem(plate, 'vpn') or getGloveItem(plate, 'weed') then 
            illegal = true
        end
    end

    if Type == 2 then
        if getTrunkItem(plate, 'vpn') or getTrunkItem(plate, 'weed') then 
            illegal = true
        end
    end
