...
RegisterNetEvent('ps-housing:client:setupSpawnUI', function(cData)
    ...
    local result = lib.callback.await('ps-housing:cb:GetOwnedApartment', false, cData.citizenid)
    if result then
        local apt = Config.Apartments[result.apartment]
        apt.property_id = result.property_id
        TriggerEvent('qb-spawn:client:setupSpawns', cData, false, {Config.Apartments[result.apartment]})
