  ...
  function getDefaultSettings(): playerStatusType {
    return {
      ...
      icons: {
        ...
        drift: getLocalStorage("drift", defaultHudIcon("drift", false, faMeteor)),
      },
      ...
      showingOrder: ["voice", "health", "armor", "hunger", "thirst", "stress", "oxygen", "armed",
        "parachute", "engine", "harness", "cruise", "nitro", "dev", "drift"],
    }
  }
  ...
  const methods = {
    ...
    receiveStatusUpdateMessage(data: playerHudUpdateMessageType) {
      update(state => {
        ...
        if (data.drift) {
          state.icons.drift.isShowing = true;
        } else {
          state.icons.drift.isShowing = false;
        }
