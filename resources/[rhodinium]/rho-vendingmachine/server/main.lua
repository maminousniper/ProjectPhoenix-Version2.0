...
RegisterNetEvent('rho_vending:setData', function(price, currency, payload)
...
RegisterNetEvent('rho_vending:SetUpStore', function(store)
    Wait(300)
...
local function SetUpHooks(inventoryFilter)
    ...
    swapHook = exports.ox_inventory:registerHook('swapItems', function(payload)
        if payload.toType == 'stash' then
            ...
            TriggerClientEvent('rho_vending:selectCurrency', payload.source, payload)
        else
            TriggerEvent('rho_vending:SetUpStore', payload.fromInventory)
        end
...
MySQL.ready(function()
    ...
    local success, error = pcall(MySQL.scalar.await, 'SELECT 1 FROM `rho_vending`')
    ...
    local result = MySQL.query.await('SELECT * FROM `rho_vending`')
    ...
    if result[1] then
        ...
        for k,v in pairs(result) do
            ...
            TriggerEvent('rho_vending:SetUpStore', v.name)
        end
...
lib.callback.register('rho_vending:fetchVendings', function(source)
    return Vending
end)
...
lib.addCommand('addvending', {
    ...
    TriggerClientEvent('rho_vending:startCreating', source, options)
end)
...
lib.addCommand('dellvending', {
    ...
    TriggerClientEvent('rho_vending:client:dellvending', source, options)
end)
...
lib.addCommand('findvending', {
    ...
    local cb = lib.callback.await('rho_vending:choseVending', source, options)
...
RegisterNetEvent('rho_vending:server:dellvending', function(shop)
    if Vending[shop] then
        MySQL.query('DELETE FROM `rho_vending` WHERE `name` = ?', { shop })

        exports.ox_inventory:ClearInventory(('stash-money-%s'):format(shop))
        exports.ox_inventory:ClearInventory(('%s'):format(shop))

        Vending[shop] = nil
        TriggerClientEvent('rho_vending:sync', -1, Vending, true)
    end
end)

RegisterNetEvent('rho_vending:buyVending', function(name)
    local src = source
    ...
    if Vending[name] then
        if exports.ox_inventory:Search(src, 'count', 'money') >= Vending[name].price then
            ...
            MySQL.update('UPDATE `rho_vending` SET `data` = ? WHERE `name` = ?', {json.encode(Vending[name], {sort_keys = true}), name})
            TriggerClientEvent('rho_vending:sync', -1, Vending, true)
...
RegisterNetEvent('rho_vending:sellVending', function(name)
    local src = source
...
RegisterNetEvent('rho_vending:createVending', function(data)
    local src = source

    MySQL.insert('INSERT INTO `rho_vending` (name, data) VALUES (?, ?)', {data.name, json.encode(data, {sort_keys = true})})
    ...
    TriggerEvent('rho_vending:SetUpStore', data.name)

    SetUpHooks(inventoryFilter)

    TriggerClientEvent('rho_vending:sync', -1, Vending, false)
end)

lib.callback.register('rho_vending:getJobs', function(source)
    local jobs = GetJobs()
    local options = {}
...
lib.callback.register('rho_vending:getGrades', function(source, job)
    local jobs = GetJobs()
    local options = {}
...
local function saveDB()
    ...
    for k,v in pairs(Vending) do
        insertTable[#insertTable + 1] = { query = 'UPDATE `rho_vending` SET `data` = ? WHERE `name` = ?', values = { json.encode(v, {sort_keys = true} ), v.name } }
    end
...
lib.versionCheck('uniqscripts/rho_vendingmachine')