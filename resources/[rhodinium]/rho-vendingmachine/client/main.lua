        ...
        options[#options + 1] = {
            ...
            onSelect = function ()
                ...
                if alert == 'confirm' then
                    TriggerServerEvent('rho_vending:buyVending', point.label)
                end
            end
        }
    end
        ...
        if point.owner == GetIdentifier() then
            options[#options+1] = {
                ...
                onSelect = function()
                    ...
                    if alert == 'confirm' then
                        TriggerServerEvent('rho_vending:sellVending', point.label)
                    end
                end
            }
...
function SetupVendings()
   ...
   local data = lib.callback.await('rho_vending:fetchVendings', false)
...
RegisterNetEvent('rho_vending:sync', function(data, clear)
...
RegisterNetEvent('rho_vending:startCreating', function(players)
    if source == '' then return end
    local vending = {}
    ...
    elseif input[4] == 'b' then
        local jobs = lib.callback.await('rho_vending:getJobs', 100)
        ...
        if not owner[1] then
            ...
            local grades = lib.callback.await('rho_vending:getGrades', 100, owner[1])
    ...
    CreateThread(function ()
        while true do
            ...
            if IsDisabledControlPressed(0, 176) then
                ...
                DeleteObject(obj)
                TriggerServerEvent('rho_vending:createVending', vending)
...
RegisterNetEvent('rho_vending:client:dellvending', function(data)
    if source == '' then return end
    ...
    TriggerServerEvent('rho_vending:server:dellvending', input[1])
end)


RegisterNetEvent('rho_vending:selectCurrency', function(payload)
    if source == '' then return end
    local itemNames = {}
    ...
    TriggerServerEvent('rho_vending:setData', input[1], input[2], payload)
end)

lib.callback.register('rho_vending:choseVending', function(options)
