...
RegisterNetEvent('rho-recycling:searchBin', function()
local time = Config.SearchTime
    if not carryingBox then
        if lib.progressBar({
            ...
            QBCore.Functions.Notify("Searching Cancelled!")
        end
    else
        QBCore.Functions.Notify("Youre already carrying a box!", "error")
    end
end)

RegisterNetEvent('rho-recycling:getReward', function()
...
local reward = rewardData.item
    if carryingBox then
        if lib.progressCircle({
            ...
            TriggerServerEvent('rho-recycling:giveitem', reward, amount)
            TriggerEvent('rho-recycling:newLoc')
            DropBox()
            carryingBox = false
        else
            QBCore.Functions.Notify("Disposing Cancelled!", "error")
        end
    else
        QBCore.Functions.Notify("Go search a bin first!", "error")
    end
end)

RegisterNetEvent('rho-recycling:punchIn')
AddEventHandler('rho-recycling:punchIn', function()
    if not punchedIn then
        ...
        QBCore.Functions.Notify("Already punched in, get started!")
    end
end)

RegisterNetEvent('rho-recycling:newLoc')
AddEventHandler('rho-recycling:newLoc', function()
