...
local function Alert(id)
    ...
    cfg.alerted = true
    TriggerServerEvent("rho-storerobbery:server:registerAlert", id, true)
...
local function RobRegistar()
    ...
    TriggerServerEvent("rho-storerobbery:server:setUse", CurrentStore, RegZoneID, true)
    ...
    if lib.progressCircle({
        ...
        local success = exports['rho-minigames']:MineSweep(prize, 10, 3, "left")
        if success then
            local itemLabel = Functions.GetItemLabel(Config.Prize.item)
            lib.callback("rho-houserobbery:server:getPrize", false, function(cb)
                Functions.Notify(
        ...
        TriggerServerEvent("rho-storerobbery:server:setUse", CurrentStore, RegZoneID, false)
    else
        TaskPlayAnim(ped, anim, "exit", 8.0, 8.0, -1, 0, 1.0, false, false, false)
        TriggerServerEvent("rho-storerobbery:server:setUse", CurrentStore, RegZoneID, false)
    end
...
local function SearchCombination(storeid, sid)
    ...
    if searchLoc.iscomputer then
        ...
        local success = exports['rho-minigames']:OpenTerminal()
        if success then
            ...
            if canGet then
                ...
                SetTimeout(1000 * TimeToWait, function()
                    lib.callback.await("rho-storerobbery:server:combination", false, storeid, sid, true)
                    Functions.Notify("You got the combination key")
                end)
            else
                lib.callback.await("rho-storerobbery:server:combination", false, storeid, sid, false)
        ...
        if lib.progressCircle({
            ...
            lib.callback.await("rho-storerobbery:server:combination", false, storeid, sid, canGet)
            if canGet then
                Functions.Notify("You got the combination key")
            else
...
local function SetupStore(id)
    ...
    local function Hack()
        ...
        TriggerServerEvent("rho-storerobbery:server:setHackUse", id, true)
        if success then
            TriggerServerEvent("rho-houserobbery:server:setHackedState", id, true)
            Wait(500)
            Alert(id)
        end
        TriggerServerEvent("rho-storerobbery:server:setHackUse", id, false)
    end
    local function OpenPinPrompt()
        ...
        if num == cfg.combination then
            lib.callback.await("rho-storerobbery:server:setSafeState", false, CurrentStore, true)
...
lib.callback.register("rho-storerobbery:client:openConfigContext", function()
    return ConfigContext()
end)

lib.callback.register("rho-storerobbery:client:resetStore", function()
    if not CurrentStore then return end
    local cfg = Config.Store[CurrentStore]
    if not cfg then return end
...
RegisterNetEvent("rho-storerobbery:client:setConfigs", function(cfg)
    Config.Store = cfg
end)

RegisterNetEvent("rho-storerobbery:client:setStoreConfig", function(id, cfg)
