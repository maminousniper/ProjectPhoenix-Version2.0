...
local function SetupStore()
    TriggerClientEvent("rho-storerobbery:client:setConfigs", -1, Config.Store)
end
...
local function Cooldown(storeid)
    ...
    TriggerClientEvent("rho-storerobbery:client:setStoreConfig", -1, storeid, cfg)
end
...
RegisterNetEvent("rho-storerobbery:server:getConfig", function()
    local src = source
    TriggerClientEvent("rho-storerobbery:client:setConfigs", src, Config.Store)
end)

RegisterNetEvent("rho-storerobbery:server:setUse", function(storeid, regid, status)
    ...
    TriggerClientEvent("rho-storerobbery:client:setStoreConfig", -1, storeid, storeConfig)
end)

RegisterNetEvent("rho-storerobbery:server:setHackUse", function(storeid, status)
    local cfg = Config.Store[storeid]
    if not cfg then return end
...
RegisterNetEvent("rho-storerobbery:server:registerAlert", function(storeid, status)
    local src = source
    local cfg = Config.Store[storeid]
    if not cfg then return end
...
RegisterNetEvent("rho-houserobbery:server:setHackedState", function(storeid, status)
    local src = source
    local cfg = Config.Store[storeid]
    if not cfg then
        warn("No config found for " .. storeid)
...
lib.callback.register("rho-houserobbery:server:getPrize", function(source, prize, storeid, regid)
    ...
    TriggerClientEvent("rho-storerobbery:client:setStoreConfig", -1, storeid, config)
    return true
end)

lib.callback.register("rho-storerobbery:server:combination", function(source, storeid, sid, status)
    local cfg = Config.Store[storeid]
    if not cfg then return end
...
lib.callback.register("rho-storerobbery:server:setSafeState", function(_, storeid)
    local config = Config.Store[storeid]
    if not config then return end
...
lib.addCommand("reset-store", {
    ...
    local sc = lib.callback.await("rho-storerobbery:client:resetStore", source)
    if sc then
        Cooldown(sc)
    end
end)
...
lib.addCommand("get-store-config", {
    ...
    ---@type number | false
    local cb = lib.callback.await("rho-storerobbery:client:openConfigContext", source)
    if cb then
