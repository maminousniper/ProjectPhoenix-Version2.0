function HighEnd(entradaCoords)
	...
	local jogos = CreateObject(`prop_kitch_juicer`,generator.x-4.688541,generator.y+6.830099,generator.z+5.80,true,false,false)
	items[#items+1] = {objects = jogos, robbed = false, anim = 'normal', delete = true, dist = 1.40}
	...
	x = x + 1
	obj[x] = mesa
	local library = CreateObject(`hei_int_heist_lng_shelf`,generator.x-1.95027700,generator.y-3.14528700,generator.z+5.300,true,false,false)
	items[#items+1] = {objects = library, robbed = false, anim = 'normal', delete = false, dist = 2.0}
	...
	FreezeEntityPosition(jogos,true)
	FreezeEntityPosition(scope,true)
	...
	SetEntityHeading(jogos,GetEntityHeading(jogos)+180)
	if Config.SpawnSafe then
		local luckyfdd = Config.SafeChance	--math.random(1,5)
		if luckyfdd == 2 then
			local safeefdd = CreateObject(`prop_ld_int_safe_01`,generator.x+5.536987,generator.y+6.348022,generator.z+0.05,false,false,false)
			items[#items+1] = {objects = safeefdd, robbed = false, anim = 'safe', delete = false, dist = 1.0}
			FreezeEntityPosition(safeefdd,true)
			SetEntityHeading(safeefdd,GetEntityHeading(safeefdd)-90)
		end
	end
...
function MidApt(entradaCoords)
	...
	items[#items+1] = {objects = sideBoard, robbed = false, anim = 'normal', delete = false, dist = 1.5}
	...
	obj[x] = toaster
	x = x + 1
	local coffeee = CreateObject(GetHashKey("prop_coffee_mac_02"), generator.x-0.04710700, generator.y-6.59617400, generator.z+2.26793200, true, false, false)
	items[#items+1] = {objects = coffeee, robbed = false, anim = 'prop_coffee_mac_02', delete = true, dist = 1.8}
	local luckyfdd = math.random(1,3)
	if luckyfdd == 2 then
	...
	if Config.SpawnSafe then
		local luckyfdd = Config.SafeChance --math.random(1,5)
		if luckyfdd == 2 then
			local safeefdd = CreateObject(`prop_ld_int_safe_01`,generator.x+6.2,generator.y+4.52972300,generator.z+1.32609800,false,false,false)
			items[#items+1] = {objects = safeefdd, robbed = false, anim = 'safe', delete = false, dist = 1.0}
			FreezeEntityPosition(safeefdd,true)
		end
	end
	...
	FreezeEntityPosition(coffeee,true)
	...
	SetEntityHeading(coffeee,GetEntityHeading(coffeee)+180)
	FreezeEntityPosition(PlayerPedId(),false)
	SetModelAsNoLongerNeeded(GetHashKey(modelo))
	Citizen.Wait(2500)
	DoScreenFadeIn(1500)
