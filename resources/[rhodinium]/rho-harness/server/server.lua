...
RegisterNetEvent('rho-harness:server:attachHarness', function(plate, ItemData)
    local src = source
    if not src then return end
    if not isVehicleOwned(plate) then return TriggerClientEvent('seatbelt:client:UseHarness', src, ItemData, true) end
    if Config.UninstallHarnessWithItem and hasHarness(plate) then return TriggerClientEvent('rho-harness:client:installHarness', src, plate, 'uninstall') end

    TriggerClientEvent('rho-harness:client:installHarness', src, plate, 'install')
end)

RegisterNetEvent('rho-harness:server:installHarness', function(plate, action)
    local src = source
...
RegisterNetEvent('rho-harness:server:uninstallHarness', function(plate)
    local src = source
    if not plate then return end
...
RegisterNetEvent('rho-harness:server:toggleBelt', function(plate, ItemData)
    local src = source
    if not src then return end
    if not hasHarness(plate) then 
        TriggerClientEvent('seatbelt:client:UseSeatbelt', src)  
        return
    end
