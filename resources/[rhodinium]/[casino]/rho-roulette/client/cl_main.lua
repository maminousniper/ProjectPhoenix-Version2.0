                ...
                while selectedRulett ~= nil do
                    ...
                    if Config.allowCustomBet then
                        if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                            local betAmountDiag = exports['qb-input']:ShowInput({
                                header = "Bet Amount",
                                submitText = "Confirm",
                                inputs = {
                                    {
                            ...
                            if tonumber(betAmountDiag.input) then
                                tmpInput = tonumber(betAmountDiag.input)
                                if tmpInput > 0 then
                                    changeBetAmount(betAmountDiag.input)
            ...
            for k, v in pairs(Rulettek) do
                if DoesEntityExist(v.tableObject) then
                    ...
                    if dist <= 2.0 then 
                        wait = 5
                        inZone  = true 
                        text = "The Diamond Casino & Resort - Roulette - Press E to sit"
                        local closestChairData = getClosestChairData(v.tableObject)
                        if closestChairData == nil then
                            break
                        end
    ...
    QBCore.Functions.TriggerCallback('roulette:server:ChipsAmount', function(result) 
        ...
        if selectedRulett == rulettIndex then
            if not statusz then
                ...
                if Config.allowCustomBet then
                    exports["qb-core"]:DrawText("Place Bet: LEFT CLICK - Adjust Bet: ↑/↓ - Exit: ← - Custom Amount: SPACEBAR")
                else
                    exports["qb-core"]:DrawText("Place Bet: LEFT CLICK - Adjust Bet: ↑/↓ - Exit: ←")
                end
            else
