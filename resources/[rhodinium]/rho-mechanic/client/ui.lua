...
local function isMenuActive(menu)
    ...
    if menu == "modMenu" then
        for _, v in pairs(Config.vehicleCustomisation) do
    ...
    elseif menu == "ResprayMenu" then
        for _, v in pairs(Config.vehicleResprayOptions) do
            if (v.category:gsub("%s+", "") .. "Menu") == currentMenu then
                menuActive = true
    ...
    elseif menu == "WheelsMenu" then
        for _, v in pairs(Config.vehicleWheelOptions) do
            if (v.category:gsub("%s+", "") .. "Menu") == currentMenu then
                menuActive = true
    ...
    elseif menu == "NeonsSideMenu" then
        for _, v in pairs(Config.vehicleNeonOptions.neonTypes) do
...
local function updateCurrentMenuItemID(id, item, item2)
    ...
    elseif currentMenu == "NeonColoursMenu" then
        local r = Config.vehicleNeonOptions.neonColours[currentMenuItemID].r
        local g = Config.vehicleNeonOptions.neonColours[currentMenuItemID].g
        local b = Config.vehicleNeonOptions.neonColours[currentMenuItemID].b
...
function InitiateMenus(isMotorcycle, vehicleHealth, categories, welcomeLabel)
    ...
    for _, v in ipairs(Config.vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if (v.id == 11 or v.id == 12 or v.id == 13 or v.id == 15) then
                if categories.mods and Config.maxVehiclePerformanceUpgrades ~= -1 then
    ...
    for _, v in ipairs(Config.vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if v.id == 11 or v.id == 12 or v.id == 13 or v.id == 15 or v.id == 16 then --Performance Upgrades
                ...
                for _, n in pairs(validMods) do
                    ...
                    if Config.maxVehiclePerformanceUpgrades == 0 then
                        -- populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. Config.vehicleCustomisationPrices.performance.prices[tempNum])
                        populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "")
            ...
            elseif v.id == 18 then
                ...
                -- populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Enable", "$" .. Config.vehicleCustomisationPrices.turbo.prices[2])

                populateMenu(v.category:gsub("%s+", "") .. "Menu", -1, "Disable", "")
                populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Enable", "")
                ...
                for _, n in pairs(validMods) do
                    -- populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. Config.vehicleCustomisationPrices.cosmetics.price)
                    populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "")
    ...
    for _, v in ipairs(Config.vehicleResprayOptions) do
    ...
    for _, v in ipairs(Config.vehicleResprayOptions) do
        ...
        for _, n in ipairs(v.colours) do
            -- populateMenu(v.category .. "Menu", n.id, n.name, "$" .. Config.vehicleCustomisationPrices.respray.price)
            populateMenu(v.category .. "Menu", n.id, n.name, "")
        end
    ...
    for _, v in ipairs(Config.vehicleWheelOptions) do
    ...
    for _, v in ipairs(Config.vehicleWheelOptions) do
        if v.id == -1 then
            ...
            -- populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Enable", "$" .. Config.vehicleCustomisationPrices.customwheels.price)
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Disable", "")
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Enable", "")
        ...
        elseif v.id ~= 20 then
            if isMotorcycle then
                if v.id == 6 then --Motorcycle Wheels
                    ...
                    for _, n in pairs(validMods) do
                        -- populateMenu(v.category .. "Menu", n.id, n.name, "$" .. Config.vehicleCustomisationPrices.wheels.price)
                        populateMenu(v.category .. "Menu", n.id, n.name, "")
                    end
    ...
    for k, v in ipairs(Config.vehicleTyreSmokeOptions) do
        -- populateMenu("TyreSmokeMenu", k, v.name, "$" .. Config.vehicleCustomisationPrices.wheelsmoke.price)
        populateMenu("TyreSmokeMenu", k, v.name, "")
    ...
    for _, v in ipairs(Config.vehicleWindowTintOptions) do
        -- populateMenu("WindowTintMenu", v.id, v.name, "$" .. Config.vehicleCustomisationPrices.windowtint.price)
        populateMenu("WindowTintMenu", v.id, v.name, "")
    ...
    if livCount > 0 then
        ...
        for i=0, livCount-1 do
            ...
            populateMenu("OldLiveryMenu", i, "Livery", "")
    ...
    for i=0, #plateTypes-1 do
        if i ~= 4 or (i == 4 and GetVehicleClass(plyVeh) == 18) or Config.allowGovPlateIndex then
            -- populateMenu("PlateIndexMenu", i, plateTypes[i+1], "$"..Config.vehicleCustomisationPrices.plateindex.price)
            populateMenu("PlateIndexMenu", i, plateTypes[i+1], "")
    ...
    for _, v in ipairs(Config.vehicleNeonOptions.neonTypes) do
    ...
    for _, v in ipairs(Config.vehicleNeonOptions.neonTypes) do
        ...
        -- populateMenu(v.name:gsub("%s+", "") .. "Menu", 1, "Enabled", "$" .. Config.vehicleCustomisationPrices.neonside.price)
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 0, "Disabled", "")
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 1, "Enabled", "")
    ...
    for k, _ in ipairs(Config.vehicleNeonOptions.neonColours) do
        -- populateMenu("NeonColoursMenu", k, vehicleNeonOptions.neonColours[k].name, "$" .. Config.vehicleCustomisationPrices.neoncolours.price)
        populateMenu("NeonColoursMenu", k, Config.vehicleNeonOptions.neonColours[k].name, "")

        if currentNeonR == Config.vehicleNeonOptions.neonColours[k].r and currentNeonG == Config.vehicleNeonOptions.neonColours[k].g and currentNeonB == Config.vehicleNeonOptions.neonColours[k].b then
    ...
    -- populateMenu("HeadlightsMenu", 1, "Enable Xenons", "$" .. Config.vehicleCustomisationPrices.headlights.price)
    populateMenu("HeadlightsMenu", 0, "Disable Xenons", "")
    populateMenu("HeadlightsMenu", 1, "Enable Xenons", "")
    ...
    for _, v in ipairs(Config.vehicleXenonOptions.xenonColours) do
        -- populateMenu("XenonColoursMenu", v.id, v.name, "$" .. Config.vehicleCustomisationPrices.xenoncolours.price)
        populateMenu("XenonColoursMenu", v.id, v.name, "")
    ...
    if state then
            ...
            elseif isMenuActive("WheelsMenu") then
                if currentWheelCategory == 20 then
                    if AttemptPurchase("wheelsmoke") then
                        local r = Config.vehicleTyreSmokeOptions[currentMenuItemID].r
                        local g = Config.vehicleTyreSmokeOptions[currentMenuItemID].g
                        local b = Config.vehicleTyreSmokeOptions[currentMenuItemID].b
        ...
        elseif isMenuActive("WheelsMenu") then
            if currentWheelCategory ~= 20 and currentWheelCategory ~= -1 then
                ...
                -- updateItem2Text(currentMenu, currentWheel, "$" .. Config.vehicleCustomisationPrices.wheels.price)
                updateItem2Text(currentMenu, currentWheel, "")
        ...
        elseif isMenuActive("NeonsSideMenu") then
            ...
            if currentMenu == "mainMenu" or currentMenu == "repairMenu" then
                ...
                RestoreNewNeonStates()
                RestoreOriginalXenonColour()
                RestoreOriginalWheels()
                RestoreOriginalColours()
                RestoreOriginalWheelsCustomNew()
                RestoreOriginalExtraNew()
                RestoreNewWindowTintOriginal()
                RestoreTurboStatus()
