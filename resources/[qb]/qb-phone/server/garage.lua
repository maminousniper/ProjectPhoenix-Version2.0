...
QBCore.Functions.CreateCallback('qb-phone:server:GetVehicleParkingGarage', function(source, cb, plate)
    local garage = MySQL.Sync.fetchScalar('SELECT garage FROM player_vehicles WHERE plate = ?', {plate})
    cb(garage)
end)
