...
RegisterNetEvent('police:client:Impound', function(fullImpound, price, days)
...
AddEventHandler('police:FullImpound', function(data)
    if data.seize then 
        local header = 'Impound Request'
        local input = lib.inputDialog(header, {
            { type = "number", label = "Fine amount", default = 250 },
        })
        ...
        local input = lib.inputDialog(header, {
            { type = "number", label = "Fine amount", default = 250 },
            { type = 'select', label = 'How many days?', options = {
                { value = '86400', label = '1 Day', icon = 'fas fa-wallet'}, 
                { value = '172800', label = '2 Days', icon = 'fas fa-landmark'},
                { value = '259200', label = '3 Days', icon = 'fas fa-landmark'},
                { value = '604800', label = '7 Days', icon = 'fas fa-landmark'},
                { value = '1209600', label = '14 Days', icon = 'fas fa-landmark'},
            }},
        })

        TriggerEvent('police:client:Impound', data.seize, input[1], input[2])
    end
end)
