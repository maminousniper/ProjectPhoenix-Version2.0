...
RegisterNetEvent('sp34r-menu', function()
    local menu = {
    {
        header = "Police Clock Service",
        isMenuHeader = true,
        icon = "fa-solid fa-circle-info"
    },
      {
        header = "Sign in",
        txt = "Go on duty and keep the peace",
        icon = "fas fa-building-shield",
        params = {
          event = 'qb-policejob:server:SetDuty',
          args = true,
        } 
      },
      {
        header = "Sign Out",
        txt = "Go off duty and eat some donuts",
        icon = "fas fa-building-shield",
        params = {
          event = 'qb-policejob:server:SetDuty',
          args = false,
...
RegisterNetEvent("qb-policejob:server:SetDuty", function(duty)
    TriggerServerEvent("qb-policejob:ToggleDuty", duty)
end)
...
RegisterNetEvent('police:client:panicbutton')
AddEventHandler('police:client:panicbutton', function()
PlayerData = QBCore.Functions.GetPlayerData()
    if PlayerData.job.name == "police" then
        if lib.progressCircle({
            duration = 2500,
            position = 'bottom',
            label = 'Notification is sent to all units',
            useWhileDead = false,
            canCancel = true,
            disable = {
                car = false,
            ...
            TriggerEvent('ps-dispatch:client:officerbackup')
        end
    end
end)
...
CreateThread(function()
    ...
    local MrpdWeapon = CreatePed(0, Config.PedModel, Config.PedLocation.x, Config.PedLocation.y, Config.PedLocation.z-1.0, Config.PedLocation.w, false, false)
	TaskStartScenarioInPlace(MrpdWeapon, 'WORLD_HUMAN_CLIPBOARD', true)
	FreezeEntityPosition(MrpdWeapon, true)
	SetEntityInvincible(MrpdWeapon, true)
	SetBlockingOfNonTemporaryEvents(MrpdWeapon, true)
end)
