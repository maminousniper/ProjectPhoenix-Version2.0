...
RegisterNetEvent('qb-policejob:ToggleDuty', function(duty)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src) 
    if not Player then return end
    if duty then 
        if Player.PlayerData.job.onduty then 
            TriggerClientEvent('QBCore:Notify', src, "Already on duty", "error")
            return
        else
            Player.Functions.SetJobDuty(true)
            TriggerClientEvent('QBCore:Notify', src, "10-04 now on duty")
        end
    elseif not duty then 
        if not Player.PlayerData.job.onduty then
            TriggerClientEvent('QBCore:Notify', src, "Already off duty", "error")
            return
        else
            Player.Functions.SetJobDuty(false)
            TriggerClientEvent('QBCore:Notify', src, "10-06 now off duty") 
        end
    end
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel, days)
    ...
    local timeImpounded = days
    
    if IsVehicleOwned(plate) then
        if not fullImpound then
            MySQL.Async.execute(
                'UPDATE player_vehicles SET state = ?, depotprice = ?, body = ?, engine = ?, fuel = ?, timestamp = ?, timeImpounded = ? WHERE plate = ?',
                {0, price, body, engine, fuel, timestamp, timeImpounded, plate})
...
QBCore.Functions.CreateUseableItem("panicbutton", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    TriggerClientEvent('police:client:panicbutton', src, item)
end)
