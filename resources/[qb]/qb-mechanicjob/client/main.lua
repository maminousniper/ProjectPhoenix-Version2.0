...
local function OpenMenu()
    ...
    local vehHealth = math.floor(GetVehicleBodyHealth(veh))
    local class = getClass(veh) 
    local props = lib.getVehicleProperties(veh)
    local name = GetVehicleName(props.model)
    local sharedVehicles = QBCore.Shared.Vehicles[name]

    --Attatch and set config for vehicle
    ...
    local openMenu = {
        {
            ...
            header = sharedVehicles.name .. " " .. sharedVehicles.brand, 
            txt = "Class ".. class, 
            params = {
...
local function PartsMenu()
    ...
    local class = getClass(Config.Plates[ClosestPlate].AttachedVehicle)
    if VehicleStatus[plate] ~= nil then
        local vehicleMenu = {
            {
                header = "Status",
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    header = v,
                    txt = Lang:t('parts_menu.status') .. percentage .. "% / 100%",
                    params = {
...
local function PartMenu(data)
    ...
    local class = data.vehClass
    local itemNeeded = getClassConfig(class, part)
...
local function RepairPart(part, class, pAmount)
    local class = class
    local PartData = getClassConfig(class, part)

    local indx = 0
    local hasItem = exports.ox_inventory:GetItemCount(PartData.item)
    if tonumber(hasItem) >= tonumber(pAmount) then
        TriggerEvent('animations:client:EmoteCommandStart', {"mechanic"})
        exports['ps-ui']:Circle(function(success)
            if success then
                QBCore.Functions.Progressbar("repair_part", Lang:t('labels.progress_bar') ..Config.ValuesLabels[part], 3500 * pAmount, false, true, {
                    ...
                    TriggerEvent('qb-vehicletuning:client:RepaireeePart', part, pAmount)
        ...
        end, pAmount + 2, 7) -- NumberOfCircles, MS 
    else
        QBCore.Functions.Notify("You dont seem to have the required items.", 'error')
    end
end
...
RegisterNetEvent("qb-mechanicjob:client:CheckStatus",function(class)
    CheckStatus(class)
end)
...
RegisterNetEvent("qb-mechanicjob:client:RepairPart",function(data)
    ...
    local class = data.vehClass
    local itemNeeded = getClassConfig(class, data.part)

    local header = 'Parts Amount - ' .. itemNeeded.item
    local input = lib.inputDialog(header, {
        { type = 'select', label = 'How many parts?', options = {
            { value = '1', label = '1 part', icon = 'fas fa-wallet'}, 
            { value = '2', label = '2 parts', icon = 'fas fa-landmark'},
            { value = '3', label = '3 parts', icon = 'fas fa-landmark'},
            { value = '4', label = '4 parts', icon = 'fas fa-landmark'},
            { value = '5', label = '5 parts', icon = 'fas fa-landmark'},
            { value = '6', label = '6 parts', icon = 'fas fa-landmark'},
            { value = '7', label = '7 parts', icon = 'fas fa-landmark'},
            { value = '8', label = '8 parts', icon = 'fas fa-landmark'},
            { value = '9', label = '9 parts', icon = 'fas fa-landmark'},
            { value = '10', label = '10 parts', icon = 'fas fa-landmark'},
        }},
    })

    --TriggerEvent('police:client:Impound', data.seize, input[1])

    RepairPart(partData, class, input[1])
end)
...
RegisterNetEvent('qb-vehicletuning:client:RepaireeePart', function(part, pAmount) 
    local veh = Config.Plates[ClosestPlate].AttachedVehicle
    local plate = QBCore.Functions.GetPlate(veh)
    local engHealth = GetVehicleEngineHealth(veh)
    local bodyHealth = GetVehicleBodyHealth(veh)
    local partStatus = VehicleStatus[plate][part]
    local class = getClass(veh)
    local itemToRemove = getClassConfig(class, part)

    if part == "engine" then
        SetVehicleEngineHealth(veh, engHealth + pAmount * 100)
        TriggerServerEvent("vehiclemod:server:updatePart", plate, "engine", partStatus + pAmount * 100, pAmount) 
        
        TriggerServerEvent("qb-mechanic:server:removeItems", itemToRemove, pAmount)

    elseif part == "body" then
        local enhealth = GetVehicleEngineHealth(veh)
        local realFuel = GetVehicleFuelLevel(veh)
        SetVehicleBodyHealth(veh, bodyHealth + pAmount * 100)
        TriggerServerEvent("vehiclemod:server:updatePart", plate, "body", partStatus + pAmount * 100, pAmount)
        SetVehicleFixed(veh)
        ...
        TriggerServerEvent("vehiclemod:server:updatePart", plate, part, partStatus + pAmount * 10, pAmount)
...
function getClassConfig(class, part)
    if class == "A" then 
        return Config.RepairCostAmountA[part]
    elseif class == "B" then 
        return Config.RepairCostAmountB[part]
    elseif class == "C" then 
        return Config.RepairCostAmountC[part]
    elseif class == "D" then 
        return Config.RepairCostAmountD[part]
    elseif class == "S" then 
        return Config.RepairCostAmountS[part]
    elseif class == "M" then 
        return Config.RepairCostAmountM[part]
    end
end