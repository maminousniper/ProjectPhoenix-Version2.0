...
RegisterCommand('+engineOn', function()
    ...
    if vehicle and DoesEntityExist(vehicle) then
        ...
        if exports['cdn-fuel']:GetFuel(vehicle) ~= 0 then
...
RegisterCommand('-engineOn', function() end, false)
...
RegisterCommand('-engineOff', function() end, false)

function TurnOnEngine(vehicle, ignoreFailure)
    if GetPedInVehicleSeat(vehicle, -1) ~= PlayerPedId() then
        return false
    end

    if HasKeys(QBCore.Functions.GetPlate(vehicle)) or AreKeysJobShared(vehicle) then

        local engineHealth = GetVehicleEngineHealth(vehicle)
        local bodyHealth = GetVehicleBodyHealth(vehicle)

        if engineHealth <= 150 or bodyHealth <= 150 then
            TriggerEvent('DoLongHudText', 'Something seems to be damaged.', 2)
            return
        end
        ...
        SetVehicleEngineOn(vehicle, true, ignoreFailure, true)
        return IsVehicleEngineOn(vehicle)
    end
end

function TurnOffEngine(vehicle, ignoreFailure)
    ignoreFailure = ignoreFailure or false
    if GetPedInVehicleSeat(vehicle, -1) ~= PlayerPedId() then
        return false
    end
    SetVehicleUndriveable(vehicle, true)
    SetVehicleEngineOn(vehicle, false, ignoreFailure, true)
    return not IsVehicleEngineOn(vehicle)
end

Citizen.CreateThread(function()
    exports["qb-keybinds"]:registerKeyMapping('', 'Vehicle', 'Engine On', '+engineOn', '-engineOn', 'IOM_WHEEL_UP', false, 'MOUSE_WHEEL') 
    exports["qb-keybinds"]:registerKeyMapping('', 'Vehicle', 'Engine Off', '+engineOff', '-engineOff', 'IOM_WHEEL_DOWN', false, 'MOUSE_WHEEL')
end)
